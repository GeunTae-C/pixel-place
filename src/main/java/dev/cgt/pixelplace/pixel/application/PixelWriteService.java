...
/*
 * HTTP controller가 붙기 전 write path의 핵심 순서를 고정하는 application service다.
 * write 성공의 1차 내구성 기준은 DB가 아니라 WAL append + fsync 성공이며,
 * 메모리 authoritative state는 WAL fsync가 성공한 뒤에만 변경한다.
 */
@Service
public class PixelWriteService {
    ...
    private void validateWriteRequest(long userId, int x, int y, int color) {
        if (userId <= 0) {
            // 유효하지 않은 사용자 write는 승인 이벤트가 아니므로 eventSeq 발급이나 WAL 기록으로 진행하면 안 된다.
            throw new IllegalArgumentException("userId must be greater than zero. userId=" + userId);
        }
        if (x < 0 || x >= BoardConstants.BOARD_SIZE) {
            // WAL에 보드 밖 좌표가 기록되면 replay가 같은 잘못된 변경을 반복하므로 append 전에 차단한다.
        ...
        if (color < 0 || color >= BoardConstants.PALETTE_SIZE) {
            // 256색 팔레트 인덱스 범위 밖 값은 1 byte 저장 모델과 복구 규칙을 깨므로 append 전에 차단한다.
