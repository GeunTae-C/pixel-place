...
/*
 * DB z=0 타일 snapshot 로딩 결과를 나타낸다.
 *
 * recovery는 이 결과를 보고 all-white 초기화와 DB snapshot 로드 중 하나를 선택한다.
 * 따라서 allMissing 플래그와 snapshots 목록은 서로 모순된 상태가 되면 안 된다.
 *
 * 이 타입은 "전부 없음"과 "무언가 로드됨"의 모순만 막는다.
 * z=0 전체 1024개 로드 여부와 중복/누락 검증은 InMemoryTileBoard 또는 TileSnapshotLoader 계층의 책임이다.
 */
