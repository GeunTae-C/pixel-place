...
// 단일 타일의 메모리 상태를 담는 최소 단위다.
// 현재는 write path 구현체가 아니라 recovery skeleton이 메모리 authoritative state를 구성하기 위한 최소 표현만 제공한다.
public record TileState(byte[] pixels, long tileVersion) {
    ...
    **/
    ...
    public TileState {
        // 타일 크기와 다른 배열이 들어오면 메모리 보드 불변식이 깨지므로 즉시 실패시킨다.
    ...
    // 새 하얀 타일 한 장을 생성한다.
    // Java byte 배열의 기본값은 0이므로, xterm 팔레트 기준 흰색 index인 DEFAULT_COLOR_INDEX로 명시적으로 채운다.
