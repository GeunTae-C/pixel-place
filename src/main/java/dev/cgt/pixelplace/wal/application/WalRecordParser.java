...
/*
 * JSON Lines WAL의 단일 라인을 복구용 레코드로 변환하고 검증한다.
 * WAL 파일의 한 줄을 WalRecord로 파싱하고 복구 가능한 정상 이벤트인지 검증한다.
 * WAL은 메모리 authoritative state를 다시 만드는 원본이므로, 형식 오류나 좌표 불일치는 조용히 건너뛰지 않고 recovery 실패로 올린다.
 */
@Component
public class WalRecordParser {
    ...
    /*
     * WAL 한 줄을 파싱한다.
     * 반환된 레코드는 MVP의 z=0, 256색 팔레트, 좌표 기반 타일 매핑 불변식을 만족해야 한다.
     */
    public WalRecord parseLine(String line, long lineNumber) {
        if (line == null || line.isBlank()) {
            // 빈 줄을 허용하면 WAL 손상과 정상 종료를 구분할 수 없으므로 복구를 중단한다.
        ...
        try {
            ...
            // JSON Lines WAL은 한 줄이 한 승인 이벤트이므로 파싱 실패는 해당 이벤트 유실 가능성으로 본다.
    ...
    private void validate(WalRecord record, long lineNumber) {
        if (record.eventSeq() <= 0) {
            // eventSeq는 replay 순서와 발급 seed의 기준이라 0 이하를 허용하지 않는다.
        ...
        if (record.z() != BoardConstants.Z0_LEVEL) {
            // MVP는 z=0 원본 타일만 authoritative state로 복구한다.
        ...
        if (record.tx() != expectedTx || record.ty() != expectedTy) {
            // WAL의 타일 좌표와 픽셀 좌표가 어긋나면 flush/replay가 서로 다른 타일을 보게 된다.
