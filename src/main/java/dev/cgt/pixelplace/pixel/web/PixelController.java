...
/*
 * POST /api/pixels HTTP 진입점이다.
 * 아직 JWT 인증 단계가 아니므로 X-User-Id를 임시 사용자 식별자로 받아 application write path에 넘긴다.
 * WAL 기록, eventSeq 발급, 메모리 타일 반영 순서는 PixelWriteService의 불변식이므로 controller는 직접 처리하지 않는다.
 */
@RestController
...
public class PixelController {
    ...
    /*
     * 픽셀 write 요청을 HTTP body와 임시 userId header에서 읽어 PixelWriteService에 위임한다.
     * 잘못된 요청은 service로 넘기기 전 필수 필드 누락을 먼저 차단해 WAL append로 진행되지 않게 한다.
     */
    @PostMapping
    ...
    /*
     * 요청 필드 누락, 좌표/색상/userId 검증 실패는 승인된 write가 아니므로 HTTP 400으로 돌려준다.
     * WAL fsync 실패 같은 IllegalStateException은 여기서 잡지 않아 서버 내부 실패로 남긴다.
     */
