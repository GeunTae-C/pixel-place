...
/*
 * 서버가 직접 발급하는 eventSeq의 단일 관리 지점
 *
 * recovery seed는 이미 DB 또는 WAL에 존재하는 마지막 eventSeq로 취급함
 * 따라서 initializeLastIssued(seed)는 다음 발급값이 아니라 마지막 발급 완료값을 저장하고,
 * allocate()는 항상 그 다음 번호(seed + 1)부터 반환해야 함
 */
@Component
public class EventSeqManager {
    ...
    public void initializeLastIssued(long seed) {
        if (seed < 0) {
            // eventSeq는 1부터 증가하는 정합성 기준이므로 음수 기준점으로 서버를 열 수 없음
    ...
    /*
     * write path가 새 이벤트를 승인하기 직전에 호출할 발급 메서드
    ...
    /*
     * recovery 검증과 운영 관측에서 현재까지 발급 완료된 마지막 eventSeq를 확인함
     * 이 값은 다음 발급값이 아니라 마지막 발급 완료값
     */
