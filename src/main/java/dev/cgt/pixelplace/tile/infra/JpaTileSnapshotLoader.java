...
// TileSnapshotLoader 포트의 실제 JPA adapter다.
// recovery 서비스가 DB read side를 JPA repository에 직접 묶지 않도록, z=0 snapshot 조회와 변환을 이 경계에서 끝낸다.
@Primary
@Component
public class JpaTileSnapshotLoader implements TileSnapshotLoader {
    ...
    public TileLoadResult loadZ0Tiles() {
        ...
        // application 계층에서 all-white pre-init 경로를 타게 한다.
        if (tileEntities.isEmpty()) {
            return TileLoadResult.allMissingResult();
        }

        // DB snapshot은 전체 존재 또는 전체 미존재만 허용한다.
        // 일부 누락은 조용한 부분 복구 대신 recovery 실패로 다뤄야 한다.
        ...
        // DB 후행 저장소 snapshot을 recovery가 사용할 TileStateSnapshot 목록으로 변환한다.
    ...
    // recovery 서비스가 DB/JPA 세부사항을 직접 모르도록 경계를 유지한다.
